...
def _logical_to_grid(logical_position):
    return (N_BOARD_PIXELS / N_ROWS) * logical_position + N_BOARD_PIXELS / (2 * N_ROWS)


class GameGUI3D(GameGUI):
    def __init__(self, game, player_x, player_o):
        self.window = Tk()
        self.window.title("Tic Tac Toe 3D")
    ...
    def draw_naught(self, logical_position):
        grid_position = _logical_to_grid(logical_position[:2])
    ...
    def click(self, event):
        grid_position = np.array([event.x, event.y], dtype=np.int_)
        logical_position = np.array(
            grid_position // (N_BOARD_PIXELS / N_ROWS), dtype=np.int_
        )
        logical_position = np.append(logical_position, 0)  # Default to the first layer
